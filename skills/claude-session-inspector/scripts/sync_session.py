...
# ///
"""Claude Stop hook entry point for incremental session refresh."""
...
IMPORTER = _load_module("claude_session_inspector_importer", "importer.py")
...
def marker_key(payload: dict[str, Any], target: str | None) -> str:
    return target or _string_field(payload, "session_id") or "unknown"
...
def select_refresh_target(claude_home: Path, payload: dict[str, Any]) -> tuple[str | None, str | None]:
    transcript_path = _string_field(payload, "transcript_path")
    ...
    try:
        return str(IMPORTER.resolve_refresh_target(claude_home, session_id)), None
    except Exception:
...
def main() -> int:
    ...
    db_path = IMPORTER.resolve_db_path(claude_home)
    try:
    ...
    try:
        IMPORTER.reconcile_pending(claude_home)
    except Exception:
        pass

    # Only import on end_turn or max_tokens; skip tool_use stops to avoid
    # running the full import on every tool call during agentic sessions.
    stop_reason = payload.get("stop_reason")
    if stop_reason not in ("end_turn", "max_tokens", None):
        print("{}", end="")
        return 0

    target, reason = select_refresh_target(claude_home, payload)
    if target is None:
        write_marker(
            db_path,
            payload,
